...
class geospatialStatistics:
    ...
    def calculate_elevation_stats(self):
        ...
        try:
            with rasterio.open(self.dem_path) as src:
                ...
                dem_crs = src.crs
                dem_nodata = src.nodata if src.nodata is not None else -999

            # Reproject vector to raster CRS
            if catchment_gdf.crs != dem_crs:
                self.logger.info(f"Reprojecting catchment from {catchment_gdf.crs} to {dem_crs}")
                catchment_gdf = catchment_gdf.to_crs(dem_crs)

            stats = zonal_stats(catchment_gdf, dem_data, affine=affine, stats=['mean'], nodata=nodata_value, boundless=False)
    ...
    def calculate_soil_stats(self):
        ...
        try:
            ...
            #self.logger.info(f"Soil Raster CRS: {raster_crs}")
            #self.logger.info(f"Soil Raster Bounds: {raster_bounds}")
            #self.logger.info(f"Soil Raster Shape: {raster_shape}")
            #self.logger.info(f"Soil Raster Nodata: {raster_nodata}")
            
            #self.logger.info(f"Catchment CRS: {catchment_gdf.crs}")
            #self.logger.info(f"Catchment Bounds: {catchment_gdf.total_bounds}")
            if catchment_gdf.crs != raster_crs:
                self.logger.info(f"Reprojecting catchments from {catchment_gdf.crs} to {raster_crs}")
                catchment_gdf = catchment_gdf.to_crs(raster_crs)

            stats = zonal_stats(catchment_gdf, soil_data, affine=affine, stats=['count'], 
                            categorical=True, nodata=nodata_value, boundless=False)
    ...
    def calculate_land_stats(self):
        ...
        try:
            ...
            # Reproject vector to raster CRS
            if catchment_gdf.crs != raster_crs:
                self.logger.info(f"Reprojecting catchment from {catchment_gdf.crs} to {raster_crs}")
                catchment_gdf = catchment_gdf.to_crs(raster_crs)
